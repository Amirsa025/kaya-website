                ...
                <div className={"direction-ltr px-3 border-t py-2 space-y-2"}>
                            ...
                            AddSkills.flatMap((item,id)=>{
                                return (
                                    <div key={id} >
                                        ...
                                        <label htmlFor={item}  className={"text-[14px]"}>{item}</label>
                                    </div>
                                )
                            })
                        }
                    ...
                    <div className={"flex flex-col justify-start w-full items-center relative"}>
                        <div className={"absolute right-3 top-4 "} onClick={handleFilterArray}>
                            <svg className="Icon-image cursor-pointer"  xmlns="http://www.w3.org/2000/svg"
                        ...
                        <input  placeholder={"add Skills"} className={"w-full p-2.5 text-gray-500 placeholder:uppercase  bg-white border rounded-md shadow-sm outline-none border rounded-md py-2 px-1 w-full  outline-0 placeholder:text-xs"} type="text" onChange={(e) => {
                                ...
                                <div  className={`w-full  border`}>
                                    ...
                                    {show && data?.slice(0,4)?.map((item:any,id:number)=>{
                                        return (
                                            ...
                                            <ul key={item.id} className={"text-sm bg-white  py-1 font-normal "}>

                                                <li className={"w-full py-2 hover:bg-blue-700 px-2 hover:text-white cursor-pointer"} onClick={()=>addSkillHandler(item?.name,item.id)}>{item.name}</li>
                                            </ul>
                                        )
                                    })}
                                </div>
