...
import {MyType} from "@/app/shared/form/filter-form/filter-type";
...
const FormFilter:React.FC<FilterForm> = ({onSubmit,handleOpen}) => {
    const [searchQuery, setSearchQuery] = useState('');
    const {data, isLoading} = useQuery(['Skills', searchQuery], () => MockAPI.search(searchQuery));
    ...
    const addSkillHandler = (name: any, id: number) => {
        const AddSkill = [...AddSkills, {id: id, name: name}]
        let uniqueArray = Array.from(new Set(AddSkill.map(obj => JSON.stringify(obj)))).map(str => JSON.parse(str));
        const uniqueSkill = Array.from(new Set(uniqueArray));
        setShow(!show)
        setAddSkills(uniqueSkill)
    }
